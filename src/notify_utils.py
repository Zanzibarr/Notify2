...
def ntf_error(msg: str, loc: str = "", line: int = "", sugg: str = "") -> None:
    ...
    [
        print(
            f"{ERROR_MSG}{'=> ' if line != first_line else ''}{line}"
            if line != ""
            else ""
        )
    ...
    if loc != "":
        print(f"{ERROR_MSG}=> Origin: {loc}:{line}")
    if sugg != "":
        ntf_info(sugg)
    exit(1)
...
def ntf_warn(msg: str, loc: str = "", line: int = "", sugg: str = ""):
    ...
    [
        print(
            f"{WARN_MSG}{'=> ' if line != first_line else ''}{line}"
            if line != ""
            else ""
        )
        for line in msg.splitlines()
    ...
    if loc != "":
        print(f"{WARN_MSG}=> Origin: {loc}:{line}")
    if sugg != "":
        ntf_info(sugg)
...
def ntf_input(msg: str, options: list[str] = []) -> str:
    ...
    if len(msg.splitlines()) > 1:
        ntf_warn("Input message should contain only one line for readability.")

    choice = input(
        f"{INPUT_MSG}{msg} {str(options).replace(',', ' |') if len(options) > 0  else ''}: "
    )
...
def ntf_exc(msg: str, loc: str = "", line: int = "") -> None:
    ...
    ntf_warn(msg=msg, loc=loc, line=line)
    raise Exception(msg)
...
class arg_parser:
    ...
    def add_param(self, alias: str, cmd_list: list[str]) -> None:
        ...
        if alias in self.__params.keys():
            ntf_error(
                f"PARSER: Alias {alias} has a duplicate.\n{OPEN_ISSUE}",
                loc=__file__,
                line=sys._getframe().f_lineno,
            )
        if any(
            p_cmd in self.__params[name]
            for p_cmd in cmd_list
            for name in self.__params.keys()
        ):
            ntf_error(
                f"PARSER: A command in {cmd_list} has a duplicate in the parser.\n{OPEN_ISSUE}",
                loc=__file__,
                line=sys._getframe().f_lineno,
            )
        if len(cmd_list) == 0:
            ntf_error(
                f"PARSER: No commands for alias {alias}.\n{OPEN_ISSUE}",
                loc=__file__,
                line=sys._getframe().f_lineno,
            )
    ...
    def parse(
        ...
        for token in argv[start:]:
            alias = next(
                (key for key in self.__params.keys() if token in self.__params[key]),
                None,
            )
            ...
            if alias is None:
                ...
                if dict:
                    if merge and last_alias != "":
                        parsed[last_alias] = " ".join(parsed[last_alias])
                    parsed[alias] = []
                    last_alias = alias
                else:
    ...
    def help(self) -> str:
        ...
        if "help" not in self.__params:
            return "First element in the list to parse is not a command."
