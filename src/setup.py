...
try:
    ...
    if any(os.path.exists(file) for file in OLD_CONFIG_PATHS):
        ...
        if ntf_input("Wish to remove old configuration files?", ["y", "n"]) == "y":
            if os.path.exists(OLD_CONFIG_PATHS[0]): subprocess.run(shlex.split(f"rm {OLD_CONFIG_PATHS[0]}"), check=True)
            if os.path.exists(OLD_CONFIG_PATHS[1]): subprocess.run(shlex.split(f"rm {OLD_CONFIG_PATHS[1]}"), check=True)
            ntf_info("Old configuration files removed.")

    if os.path.exists(OLD_DEST_PATH):

        if ntf_input("Wish to remove old version of notify?", ["y", "n"]) == "y":
            subprocess.run(shlex.split(f"rm -r {OLD_DEST_PATH}"), check=True)
