...
def write_conf_profile_from_dict(name, profile): 
	...
	if "disable_web_page_preview" not in profile: utils.ntf_exc("NOTIFY_EXCEPTION: Missing 'disable_web_page_preview' in the profile.")
	if "disable_notification" not in profile: utils.ntf_exc("NOTIFY_EXCEPTION: Missing 'disable_notification' in the profile.")
	if "protect_content" not in profile: utils.ntf_exc("NOTIFY_EXCEPTION: Missing 'protect_content' in the profile.")
	if "allow_sending_without_reply" not in profile: utils.ntf_exc("NOTIFY_EXCEPTION: Missing 'allow_sending_without_reply' in the profile.")
	if "parse_mode" not in profile: utils.ntf_exc("NOTIFY_EXCEPTION: Missing 'parse_mode' in the profile.")
...
def set_default_profile(name):
	...
	if name not in configuration["profiles"]:
		utils.ntf_exc("NOTIFY_EXCEPTION: The specified name was not in the list of profiles in the configuration file.")
...
class bot:
		...
		if token == "" and name == "":
			...
			if name not in configuration:
				...
				if not valid_token:
					...
					if not valid_profile:
						utils.ntf_warn(f"the token inside the profile {name} is invalid.\nRest of the profile loaded successfully, used token specified.")
	...
	def update_progress_bar(self): 
		...
		if not self.__pb.active: utils.ntf_exc("NOTIFY_EXCEPTION: Progress bar hasn't been created or has already terminated.")
	...
	def on(self): 
		...
		if not self.__env: utils.ntf_exc("NOTIFY_EXCEPTION: constructor hasn't been called yet.")
	...
	def send_message_by_text(self, text, chat_id="", message_thread_id="", parse_mode="", disable_web_page_preview="", disable_notification="", protect_content="", reply_to_message_id="", allow_sending_without_reply=""):
		...
		if chat_id == "":
			utils.ntf_exc("NOTIFY_EXCEPTION: Either set a to_chat_id in the profile or specify one as a parameter.")
		if disable_web_page_preview == "":
	...
	def forward_message(self, message_id, chat_id, from_chat_id="", message_thread_id="", disable_notification="", protect_content=""):
		...
		if from_chat_id == "":
			utils.ntf_exc("NOTIFY_EXCEPTION: Either set a from_chat_id in the profile or specify one as a parameter.")
		if disable_notification == "":
			disable_notification = self.__profile["disable_notification"]
		if protect_content == "":
			protect_content = self.__profile["protect_content"]
	...
	def send_photo_by_path(self, file_path, chat_id = "", message_thread_id="", caption="", parse_mode="", has_spoiler="", disable_notification="", protect_content="", reply_to_message_id="", allow_sending_without_reply=""):
		...
		if not os.path.exists(file_path):
			utils.ntf_exc(f"NOTIFY_EXCEPTION: The file_path {file_path} doesn't lead to any file.")
	...
	def send_audio_by_path(self, file_path, chat_id="", message_thread_id="", caption="", parse_mode="", duration="", performer="", title="", thumbnail="", disable_notification="", protect_content="", reply_to_message_id="", allow_sending_without_reply=""):
		...
		if thumbnail != "" and not os.path.exists(thumbnail):
			utils.ntf_exc(f"NOTIFY_EXCEPTION: The file_path {thumbnail} doesn't lead to any file.")
	...
	def edit_message_text(self, text, chat_id="", message_id="", inline_message_id="", parse_mode="", disable_web_page_preview=""):
		...
		if inline_message_id == "":
			...
			if message_id == "" or chat_id == "":
				utils.ntf_exc("NOTIFY_EXCEPTION: If the inline_message_id is not defined, both chat_id (default counts) and message_id must be defined.")
		else:
			chat_id = ""
			message_id = ""
		if disable_web_page_preview == "":
	...
	def delete_message(self, message_id, chat_id=""):
		...
		if chat_id == "":
			chat_id = self.__profile["to_chat_id"]
		if chat_id == "":
			utils.ntf_exc("NOTIFY_EXCEPTION: Either set a chat_id in the profile or specify one as a parameter.")
